...
между соседними поверхностями. Дневная тема уже low-glare, а ночная сохраняет ту же цветовую логику на
тёмном сине-графитовом фоне.
...
| --- | --- | --- |
| `--color-canvas` | `#CFCEC6` | Основной low-glare фон |
| `--color-surface` | `#DEDBD4` | Нейтральные карточки и панели |
...
Тему можно выбрать вручную или оставить системной. Выбор сохраняется на устройстве.
...
| `--color-raspberry` | `#E19AA5` |
| `--color-ink` | `#EBE6DD` |
| `--color-ink-muted` | `#AEBDC5` |

При первой загрузке тема применяется до отрисовки интерфейса, чтобы не возникала белая вспышка.
